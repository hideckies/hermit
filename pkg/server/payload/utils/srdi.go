// Convert DLL to Shellcode for Shellcode Reflective DLL Injection.
// References:
//  - https://github.com/BishopFox/sliver/blob/d0e5fbb68081ea8853e4cb709cffb19fec1f675d/server/generate/srdi.go#L4
...
func calcHashFunc(funcName string) uint32 {
	funcBytes := []byte(funcName)
	funcBytes = append(funcBytes, 0x00)

	funcHash := uint32(0)

	for _, b := range funcBytes {
		funcHash = ror(funcHash, 13, 32)
		funcHash += uint32(b)
	}
...
// Go implementation of python's struct.pack function
...
func dllToShellcode(dllPath string, funcName string, userdata string) (shellcode []byte, err error) {
	...
	var hashFunc []byte
	if funcName != "" {
		hashFuncUint32 := calcHashFunc(funcName)
		hashFunc = pack(hashFuncUint32)
	} else {
		hashFunc = pack(uint32(0x10))
	}
	...
	shellcode = convertToShellcode(dllBytes, hashFunc, userData, flags)
	return shellcode, nil
}

// Generates sRDI shellcode from DLL
func GenerateSRDIShellcode(dllPath string, funcName string) (shellcodePath string, err error) {
	shellcode, err := dllToShellcode(dllPath, funcName, "")
	if err != nil {
		return "", err
	}
