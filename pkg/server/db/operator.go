...
func (d *Database) OperatorAdd(ope *operator.Operator) error {
	...
	stmt, err := tx.Prepare("INSERT INTO operator (uuid, name) VALUES (?, ?)")
	if err != nil {
		return err
	}
	defer stmt.Close()

	_, err = stmt.Exec(ope.Uuid, ope.Name)
	if err != nil {
		return err
	}
...
func (d *Database) OperatorGetById(operatorId uint) (*operator.Operator, error) {
	stmt, err := d.DB.Prepare("SELECT id, uuid, name FROM operator WHERE id = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
	...
	err = stmt.QueryRow(operatorId).Scan(&id, &uuid, &name)
	if err != nil {
		return nil, err
	}
	return operator.NewOperator(uint(id), uuid, name), nil
}

func (d *Database) OperatorGetByUuid(operatorUuid string) (*operator.Operator, error) {
	stmt, err := d.DB.Prepare("SELECT id, uuid, name FROM operator WHERE uuid = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
	...
	err = stmt.QueryRow(operatorUuid).Scan(&id, &uuid, &name)
	if err != nil {
		return nil, err
	}
	return operator.NewOperator(uint(id), uuid, name), nil
...
func (d *Database) OperatorGetAll() ([]*operator.Operator, error) {
	rows, err := d.DB.Query("SELECT id, uuid, name FROM operator")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err = rows.Scan(&id, &uuid, &name)
		if err != nil {
			return nil, err
		}

		op := operator.NewOperator(uint(id), uuid, name)
		ops = append(ops, op)
	}
	return ops, nil
}
