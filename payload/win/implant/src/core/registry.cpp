...
std::wstring GetRegSubKeys(
    ...
    HKEY hRootKey = GetRegRootKey(wRootKey);
    if (hRootKey == NULL)
    {
        return L"Error: Could not get root key.";
    }

    std::vector<std::wstring> vSubKeys = ListRegSubKeys(
        hRootKey,
        wSubKey,
        KEY_READ,
        bRecurse
...
std::wstring GetRegValues(
    ...
    std::vector<std::wstring> vNewSubKeys = ListRegSubKeys(
        hRootKey,
        wSubKey,
        KEY_READ,
        bRecurse
        ...
        do {
            ...
            if (lStatus == ERROR_SUCCESS)
            {
                ...
                if (dwType == REG_DWORD)
                {
                    ...
                    result += ConvertDWORDToWstring(_dwValue) + L"\n";
                }
