...
std::wstring GetNetTCPConnection()
{
    ...
    if ((dwRetVal = GetTcpTable(pTcpTable, &ulSize, TRUE)) == NO_ERROR)
    {
        ...
        for (i = 0; i < (int)pTcpTable->dwNumEntries; i++)
        {
            ...
            std::wstring wLocalAddr = UTF8Decode(sLocalAddr);
            result += wLocalAddr;
            ...
            std::wstring wLocalPort = UTF8Decode(sLocalBuffer);
            if (wLocalPort == L"")
            {
                result += L"0\t\t";
            }
            ...
            std::wstring wRemoteAddr = UTF8Decode(sRemoteAddr);
            result += wRemoteAddr;
            ...
            std::wstring wRemotePort = UTF8Decode(remoteBuffer);
            if (wRemotePort == L"")
            {
                result += L"0\t\t";
            }
    ...
    {
        FREE(pTcpTable);
        return L"Error: GetTcpTable2 failed with " + ConvertDWORDToWstring(dwRetVal);
    }
